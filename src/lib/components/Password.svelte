...
<tr class="bg-slate-600 even:bg-slate-700">
	<td>
		<a class="hover:underline hover:text-blue-600" href="https://www.{website}">{website}</a></td
	>
	...
	{#if showPassword}
		<td
			><button on:click={() => navigator.clipboard.writeText(password)} class="hover:underline"
				>{password}</button
			></td
		>
	{:else}
